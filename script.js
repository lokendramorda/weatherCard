...
const tempEl = document.getElementById('tempi');
...
setInterval(() => {
	...
	const mintutes = time.getMinutes();
	const ampm = hour >=12 ? 'Pm' : "Am"

  timeEl.innerHTML = hoursIn12HrFormat + ':' + mintutes+ ' '  + ampm

	dateEl.innerHTML = days[day] + ' ' + date+ ' '  + months[month]

},1000);

const API_KEY = '6b6726b8929179b7c9fa0d982a382789'
...
function getWeatherData (){
	navigator.geolocation.getCurrentPosition((success) => {
		...
		.then(data => {
			...
			showWeatherData(data.list[0].main);
		})
...
function showWeatherData(data){
	let { humidity, temp_max, temp_min, sea_level, temp} = data;

    currentWeatherItemEl.innerHTML =	`<div class="bedge" id="Humidity">Humidity : ${humidity} </div>

			 <div class="Pressure" id="Pressure">Sea level : ${sea_level} </div>

					<div class="Pressure" id="Pressure">${temp_max} - ${temp_min}</div>`;

     tempEl.innerHTML = `<div style="  font-size: 60px;
		   line-height: 60px;"> ${temp}</div>`
